...
* Use currying, partial application, and data-last function signatures where they improve composability.
* Prefer expression-oriented programming (e.g., ternaries or pattern matching) over statement-based execution (if/else blocks).


* **Data over Objects (No OOP):**
* Do not use classes, inheritance, encapsulation, or the `this` keyword. *(Exception: when strictly mandated by a framework lifecycle or library).*
...
* Prefer slight duplication over the wrong abstraction. Abstract only after a clear, stable pattern emerges multiple times.
* YAGNI (You Ain't Gonna Need It): Do not build features or abstractions "just in case."


* **Strict, Explicit Typing:**
* Explicitly type all function arguments and return values.
* Avoid overly broad types unless they are genuinely required by the interface or framework.
* Define specific shapes for all data structures (e.g., TS `interface` or Python `TypedDict`/`dataclasses`).


* **Self-Documenting Code:**
* Code must explain *what* it is doing through clear variable and function names.
* Use inline comments exclusively to explain *why* a non-obvious decision was made.
* Use standard docstrings or API documentation formats for public modules and functions.


* **Tooling & Error Handling:**
* Code must pass standard formatters and linters for the target language.
